...
struct PlateHasherAll {
    size_t operator()(const VehiclePlate& plate) const {
        ...
        // Обрабатываем буквы
        for (char letter : plate.GetLetters()) {
            hash = hash * 31 + (letter - 'A'); // Сдвигаем и добавляем значение буквы
        }
        ...
        return static_cast<size_t>(hash); // Приводим к size_t для возвращаемого значения
    }
};
