...
// Основной метод для отрисовки карты
void MapRenderer::Render(std::ostream& out) const {
    ...
    // Если есть остановки, принадлежащие маршрутам, и требуется их отрисовка
    if (settings_.render_stops && !stops_in_routes.empty()) {
        DrawStops(doc, projector);
    }
