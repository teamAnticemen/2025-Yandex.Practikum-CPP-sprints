...
void StatReader::ProcessQuery(const std::string_view& name) const {
    if (name.substr(0, 4) == "Bus ") {
        ...
        double curvature = bus_info.curvature; // Извилистость маршрута
        ...
        const auto& buses = catalogue_.GetBusesByStop(stop_name); // Получаем автобусы для остановки по константной ссылке
        ...
        if (buses.empty()) {
            ...
            // Извлекаем имена автобусов из указателей и добавляем их в вектор
            for (const transport_catalogue::Bus* bus : buses) {
                sorted_buses.push_back(bus->name);
            }

            std::sort(sorted_buses.begin(), sorted_buses.end()); // Сортируем маршруты
