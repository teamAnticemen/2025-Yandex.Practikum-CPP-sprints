...
/*void TestInsert() {
    cout << "TestInsert"s << endl;
    const auto num_elements = 10000000;
    SimpleVector<size_t> vec;

    // Вставка элементов
    for (size_t i = 0; i < num_elements; ++i) {
        vec.Insert(i, static_cast<int>(i));
    }

    // Проверка размера вектора
    assert(vec.Size() == num_elements);

    // Проверка корректности вставленных значений
    for (size_t i = 0; i < num_elements; ++i) {
        assert(vec[i] == static_cast<long unsigned int>(i));
    }

    std::cout << "Done! " << num_elements << " elements were inserted correctly!" << std::endl;
}*/
