...
public:
    ...
    PtrVector(const PtrVector& other) {
        for (const auto& item : other.items_) {
            if (item) {
                items_.push_back(new T(*item)); // Создание копии объекта
            } else {
                items_.push_back(nullptr); // Нулевой указатель остается нулевым
            }
        }
    }
    ...
    ~PtrVector() {
        for (auto item : items_) {
            delete item; // Удаление объекта на который указывает указатель
        }
    }
