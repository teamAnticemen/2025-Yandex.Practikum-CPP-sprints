...
public:
    ...
    explicit ArrayPtr(size_t size) : data_(new T[size]), size_(size) {}
    ...
    ArrayPtr(ArrayPtr&& other) noexcept 
        ...
        other.data_ = nullptr; // Обнуляем указатель перемещенного объекта
        other.size_ = 0; // Обнуляем размер перемещенного объекта
    }

    ArrayPtr& operator=(ArrayPtr&& other) noexcept {
        if (this != &other) {
            delete[] data_; // Удаляем предыдущие данные
            data_ = other.data_;
            size_ = other.size_;
            other.data_ = nullptr; // Обнуляем указатель перемещенного объекта
            other.size_ = 0; // Обнуляем размер перемещенного объекта
    ...
    ~ArrayPtr() { delete[] data_; } // Добавляем деструктор для удаления данных
